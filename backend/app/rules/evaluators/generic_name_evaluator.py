...
class GenericNameEvaluator(BaseRuleEvaluator):
    ...
    def evaluate(
        ...
        # Fallback to product name hint or text block check
        product_name_hint = product_context.get("name")
        
        found_val = extracted_val or product_name_hint

        if not found_val or len(found_val.strip()) < 3:
            return (
                "REVIEW",
                None,
                expected_cond,
                "Generic or common commodity name could not be reliably verified from the artwork.",
                None,
