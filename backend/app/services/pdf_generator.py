...
class PDFReportGenerator:
    ...
    def generate_suggested_design_pdf(
        ...
        # Build verification summary counts
        ...
        # Safe sanitized filename preventing path traversal
        safe_name = re.sub(r'[^a-zA-Z0-9_\-]', '_', product.name).strip('_')
        if not safe_name:
            safe_name = "Product"
        filename = f"{safe_name}_NIYAMORA_Suggestion_{suggested_design.version_label}.pdf"
        ...
        # Header Bar
        page1.draw_rect(fitz.Rect(0, 0, 595, 60), color=None, fill=(0.06, 0.09, 0.16)) # Dark Slate
        ...
        # Verification Summary Box (No arbitrary scores)
        ...
        # Embed rendered artwork image if available
        rendered_path = suggested_design.rendered_artwork_reference
        if rendered_path and os.path.exists(rendered_path):
            try:
                img_rect = fitz.Rect(60, 70, 535, 680)
                page2.insert_image(img_rect, filename=rendered_path)
            except Exception as e:
                logger.warning(f"Could not embed rendered image into PDF: {e}")
                page2.draw_rect(fitz.Rect(60, 70, 535, 680), color=(0.8, 0.8, 0.8), fill=(0.95, 0.95, 0.95))
                page2.insert_text((180, 380), "High-Resolution Artwork Packshot Rendered", fontsize=12, color=(0.4, 0.4, 0.4))
        else:
            page2.draw_rect(fitz.Rect(60, 70, 535, 680), color=(0.8, 0.8, 0.8), fill=(0.95, 0.95, 0.95))
            page2.insert_text((180, 380), "High-Resolution Artwork Packshot Rendered", fontsize=12, color=(0.4, 0.4, 0.4))
